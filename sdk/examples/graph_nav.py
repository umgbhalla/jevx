...
def one_hop(g: Graph, node: str, goal: dict, s1) -> tuple[list[tuple[float, dict]], bool]:
    ...
    n = g.nodes[node]
    state = {"current_node": {"id": node, "label": n["label"],
    ...
    for i, e in enumerate(nbrs):
        ...
        keys.append((k, e))
        t = g.nodes[e["dst"]]
        crit[k] = (f"{e['rel']} -> {t['label']} "
                   f"{clean_props(t.get('props', {}), t.get('identity', []))}")
    c = pick(f"Choose the single outgoing relationship advancing this goal: {goal['description']}",
             {**state, " hop": len(keys)}, crit, client=s1)
    ...
    while frontier and calls < max_calls:
        ...
        for b in frontier:
            ...
            for p, e in take:
                ...
                if dst in b["visited"]:
                    continue
                nxt.append({"node": dst, "path": b["path"] + [dst],
                            "score": b["score"] + math.log(max(p, 1e-12)),
                            "visited": b["visited"] | {dst}, "depth": b["depth"] + 1})
        if not nxt:
            return {"status": "NO_CANDIDATES", "best": frontier[0]["path"], "calls": calls}
        frontier = sorted(nxt, key=lambda b: b["score"], reverse=True)[:beam]
    best = max(frontier, key=lambda b: b["score"]) if frontier else None
...
def demo_graph() -> Graph:
    ...
    g.edges = {"acme": [{"rel": "BILLED", "props": {}, "dst": "inv1"},
